...
func LoadFileList(path string) (*FileList, error) {
	payload, err := decompressFileList(path)
	if err != nil {
		return nil, err
	}
...
func ParseFileList(path string) error {
	fl, err := LoadFileList(path)
	if err != nil {
		return err
	}
	dumpFileList(fl)
...
func decompressFileList(path string) ([]byte, error) {
...
func parseFileItems(buf []byte, header CacheContext) ([]FileItem, int, error) {
	...
	for offset < limit {
		...
		if item.DataOffset != 0 {
			...
			item.ExtraData = buf[extraStart:extraEnd]
		}
...
func dumpFileList(fl *FileList) {
	fmt.Printf("Cache Context:\n")
	fmt.Printf("  cbSize: %d\n", fl.Header.CBSize)
	fmt.Printf("  localFlags: %d\n", fl.Header.LocalFlags)
	fmt.Printf("  channelSupport: %d\n", fl.Header.ChannelSupport)
	fmt.Printf("  channelIndex: %d\n", fl.Header.ChannelIndex())
	fmt.Printf("  flags: %d\n", fl.Header.FlagsValue())
	fmt.Printf("  signOrLen: 0x%08X\n", fl.Header.SignOrLen)
	fmt.Printf("  appId: %d\n", fl.Header.AppID)
	fmt.Printf("  fileId: %d\n", fl.Header.FileID)
	fmt.Printf("  fileSize: %d\n", fl.Header.FileSize)
	fmt.Printf("  sizeOfHeader: %d\n", fl.Header.SizeOfHeader)
	fmt.Printf("  sha: %s\n\n", hex.EncodeToString(fl.Header.Sha[:]))

	fmt.Printf("Files (%d entries):\n", len(fl.Items))
	for i, item := range fl.Items {
		kind := "FILE"
		if item.FileAttributes&0x10 != 0 {
			kind = "DIR"
		}
		created := "-"
		if t := filetimeToTime(item.CreationTime); !t.IsZero() {
			created = t.UTC().Format(time.RFC3339)
		}
		fmt.Printf("  [%d] %s %s\n", i, kind, item.Name)
		fmt.Printf("      flags=0x%X attrs=0x%X size=%d hash=%s created=%s\n",
			item.Flags,
			item.FileAttributes,
			item.Context.FileSize,
			hex.EncodeToString(item.Context.Sha[:]),
			created,
		)
	}

	if len(fl.Extensions) > 0 {
		fmt.Printf("\nExtensions (%d entries):\n", len(fl.Extensions))
		for _, ext := range fl.Extensions {
			fmt.Printf("  type=0x%X size=%d\n", ext.Type, len(ext.Data))
		}
	}
}
...
func filetimeToTime(ft int64) time.Time {
	...
	sec := ft/ticksPerSecond - unixToFiletime
